...
function validarImagen(file, maxMb) {
  if (!file) return "Selecciona un archivo.";
  if (!file.type.startsWith("image/")) return "El archivo debe ser una imagen (JPG, PNG o WEBP).";
  if (file.size > maxMb * 1024 * 1024) {
    return `La imagen pesa demasiado (${(file.size / 1024 / 1024).toFixed(1)}MB). El máximo permitido es ${maxMb}MB.`;
  }
  return null;
}

/** Aviso flotante, siempre visible arriba de la pantalla sin importar el scroll */
function Toast({ mensaje }) {
  if (!mensaje) return null;
  return (
    <div
      className={`fixed top-4 left-1/2 -translate-x-1/2 z-50 px-5 py-3 rounded-lg shadow-lg text-sm font-semibold max-w-[92vw] text-center ${
        mensaje.tipo === "ok" ? "bg-primary text-white" : "bg-red-600 text-white"
      }`}
    >
      {mensaje.texto}
    </div>
...
function ConfiguracionContenido() {
  const [tab, setTab] = useState("vacacional");
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-4 py-6">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {tab === "vacacional" ? (
          <TabVacacional mostrarMensaje={mostrarMensaje} />
        ) : (
          <TabGeneral mostrarMensaje={mostrarMensaje} />
        )}
      </div>
    </div>
  );
...
/* ============================================================
   ...
   ============================================================ */
function TabVacacional({ mostrarMensaje }) {
  ...
  // Estado de selección de imágenes (aún no subidas)
  ...
  useEffect(() => {
    ...
    // Al cambiar de edición, descarta cualquier imagen seleccionada sin guardar
    setLogoFile(null);
    setLogoPreview(null);
    setHeaderFile(null);
  ...
  const seleccionarLogo = (file) => {
    const err = validarImagen(file, MAX_LOGO_MB);
    if (err) {
  ...
  const guardarLogo = async () => {
    ...
    const { error: errUpload } = await supabase.storage.from("logos").upload(path, logoFile, { upsert: true });

    if (errUpload) {
      mostrarMensaje(
        "error",
        "Error al subir el logo. Es probable que falten permisos en Supabase Storage (ejecuta migracion_storage_policies.sql)."
      );
    ...
    // Se agrega ?v= para evitar que el navegador muestre la imagen vieja en caché
    const urlFinal = `${urlData.publicUrl}?v=${Date.now()}`;

    await supabase.from("ediciones").update({ logo_url: urlFinal }).eq("id", edicionId);
    setEdicionActual((prev) => ({ ...prev, logo_url: urlFinal }));
    mostrarMensaje("ok", "Logo del vacacional actualizado. Ya se ve en el header.");
    setLogoFile(null);
    setLogoPreview(null);
    setGuardandoLogo(false);
  };
  ...
  const seleccionarHeader = (file) => {
    const err = validarImagen(file, MAX_HEADER_MB);
    if (err) {
      mostrarMensaje("error", err);
      return;
    }
  ...
  const guardarHeader = async () => {
    ...
    const { error: errUpload } = await supabase.storage.from("logos").upload(path, headerFile, { upsert: true });

    if (errUpload) {
      mostrarMensaje(
        "error",
        "Error al subir la imagen de portada. Es probable que falten permisos en Supabase Storage (ejecuta migracion_storage_policies.sql)."
      );
    ...
    await supabase.from("ediciones").update({ imagen_header_url: urlFinal }).eq("id", edicionId);
    setEdicionActual((prev) => ({ ...prev, imagen_header_url: urlFinal }));
    mostrarMensaje("ok", "Imagen de portada actualizada. Ya se ve en el header.");
    setHeaderFile(null);
    setHeaderPreview(null);
    setGuardandoHeader(false);
  };
      ...
      {edicionActual && (
        <>
          ...
          <section className="bg-white border rounded-xl p-4 mb-6">
            ...
            <p className="text-gray-400 text-xs mb-3">
              📐 <strong>Forma:</strong> cuadrada (1:1) &middot; <strong>Mínimo:</strong> 300×300px (ideal 500×500px)
              &middot; <strong>Formato:</strong> JPG, PNG o WEBP &middot; <strong>Peso máximo:</strong> {MAX_LOGO_MB}MB.
              Se muestra redondo en el header.
            </p>

            <div className="flex flex-wrap items-center gap-4 mb-3">
              {/* Vista previa: la nueva imagen elegida, o si no hay ninguna, la que ya está guardada */}
              {(logoPreview || edicionActual.logo_url) && (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  src={logoPreview || edicionActual.logo_url}
                  alt="Logo del vacacional"
                  className="h-20 w-20 rounded-full object-cover border-2 border-primary"
                />
              )}
              <input type="file" accept="image/*" onChange={(e) => seleccionarLogo(e.target.files?.[0])} />
            </div>
            ...
            {logoFile && (
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  className="bg-primary hover:bg-primary-dark text-white text-sm font-semibold px-4 py-2 rounded-lg disabled:opacity-50"
                >
                ...
                <button
                  ...
                  disabled={guardandoLogo}
                  className="text-gray-500 text-sm font-semibold px-4 py-2"
                >
                  Cancelar
                </button>
              </div>
          ...
          <section className="bg-white border rounded-xl p-4 mb-6">
            ...
            <p className="text-gray-400 text-xs mb-3">
              📐 <strong>Forma:</strong> rectangular horizontal (ancho mayor que alto) &middot;{" "}
              <strong>Mínimo:</strong> 1200×400px &middot; <strong>Formato:</strong> JPG, PNG o WEBP &middot;{" "}
              <strong>Peso máximo:</strong> {MAX_HEADER_MB}MB. Se ve de fondo detrás del título con un filtro verde
              oscuro encima para que el texto siempre se lea bien. Si no subes ninguna, se usa el fondo verde por
              defecto.
            </p>

            {(headerPreview || edicionActual.imagen_header_url) && (
              <div className="mb-3">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={headerPreview || edicionActual.imagen_header_url}
            ...
            <div className="flex flex-wrap items-center gap-3 mb-2">
              <input type="file" accept="image/*" onChange={(e) => seleccionarHeader(e.target.files?.[0])} />
              {!headerFile && edicionActual.imagen_header_url && (
            ...
            {headerFile && (
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  className="bg-primary hover:bg-primary-dark text-white text-sm font-semibold px-4 py-2 rounded-lg disabled:opacity-50"
                >
                  {guardandoHeader ? "Guardando..." : "💾 Guardar portada"}
                </button>
                <button
                  onClick={cancelarHeader}
                  disabled={guardandoHeader}
                  className="text-gray-500 text-sm font-semibold px-4 py-2"
                >
                  Cancelar
...
/* ============================================================
   ...
   ============================================================ */
function TabGeneral({ mostrarMensaje }) {
  ...
  const guardarLogoIglesia = async () => {
    if (!logoFile) return;
    setGuardandoLogo(true);
    ...
    if (errUpload) {
      mostrarMensaje(
        "error",
        "Error al subir el logo de la iglesia. Es probable que falten permisos en Supabase Storage (ejecuta migracion_storage_policies.sql)."
      );
      setGuardandoLogo(false);
      return;
    }
    ...
    await supabase.from("configuracion_general").update({ logo_iglesia_url: urlFinal }).eq("id", config.id);
    setConfig((prev) => ({ ...prev, logo_iglesia_url: urlFinal }));
  ...
  return (
    <section className="bg-white border rounded-xl p-4 mb-6">
      ...
      <p className="text-gray-400 text-xs mb-4">
        Estos datos NO cambian entre ediciones/años del vacacional. Se muestran siempre en el footer, en la sección
        &quot;Organiza&quot;.
      </p>
      ...
      <div>
        ...
        <div className="flex flex-wrap items-center gap-4 mb-3">
          {(logoPreview || config.logo_iglesia_url) && (
            // eslint-disable-next-line @next/next/no-img-element
            <img
              src={logoPreview || config.logo_iglesia_url}
